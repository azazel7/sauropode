...
globalkeys = awful.util.table.join(
    ...
    function () 
		  if client.focus then
			  --TODO change someday but for now, awful.tag.index doesn't work
			  ...
			  local nexttag = client.focus.screen.tags[nextidx]
			  local prevtag = client.focus.screen.tags[curidx]
			  if tag then
				  client.focus:toggle_tag(nexttag)
				  client.focus:toggle_tag(prevtag)
			  end
			  awful.tag.viewprev()
		 end
    end),
    ...
    -- Widgets popups
    awful.key({ modkey, }, "c", function () os.execute("slock") end),
    ...
    awful.key({ modkey }, "F9",
        function ()
            ...
            os.execute("pamixer --increase 4")
            beautiful.volume.update()
        end,
	  {description = "Increase volume", group = "Alsa"}),
    awful.key({ modkey }, "F8",
        function ()
            os.execute("pamixer --decrease 4")
            beautiful.volume.update()
        end,
    ...
    awful.key({ modkey, "Control" }, "space",
	  ...
	  {description = "Toggle play", group = "MPD"}),
    awful.key({ modkey, "Control" }, "F8",
        function ()
            awful.spawn.with_shell("mpc volume -7")
            beautiful.mpd.update()
        end,
	  {description = "Increase MPD volume", group = "MPD"}),
    awful.key({ modkey, "Control" }, "F9",
        function ()
            awful.spawn.with_shell("mpc volume +7")
            beautiful.mpd.update()
        end,
	  {description = "Decrease MPD volume", group = "MPD"}),
	...
	function () 
	  ...
	  {description = "Delete current song from playlist", group = "MPD"}),
    ...
    awful.key({ modkey }, "e", function () awful.spawn(file_manager) end,
	  {description = "Start ranger", group = "launcher"}),
    awful.key({ 	 	  }, "Print", function ()
		repeat
			file = os.getenv("HOME") .. "/screenshot" .. screenshot_counter .. ".png"
			screenshot_counter = screenshot_counter + 1
		until file_exists(file) == false
		naughty.notify({timeout=7,
					 title = "Screenshot",
					 text = file })
		
		awful.util.spawn(screenshot .. " " .. file)
	end),
...
clientkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "f",
        ...
        {description = "toggle fullscreen (useless)", group = "client"}), -- on a déjà super+m pour maximize
